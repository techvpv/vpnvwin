    ...
    Protected Overrides Sub OnMouseDown(ByVal e As MouseEventArgs)
        ...
        If Not (_IsParentForm AndAlso ParentForm.WindowState = FormWindowState.Maximized OrElse _ControlMode) Then
            If _Movable AndAlso Frame.Contains(e.Location) Then
                If Not New Rectangle(Width - 22, 5, 15, 15).Contains(e.Location) AndAlso Not New Rectangle(Width - 44, 5, 15, 15).Contains(e.Location) Then
                    Capture = False
                End If
...
Class ChromeThemeContainer
    ...
    Protected Overrides Sub OnMouseClick(ByVal e As MouseEventArgs)
        If New Rectangle(Width - 44, 5, 15, 15).Contains(New Point(X, Y)) Then
            Me.FindForm.WindowState = FormWindowState.Minimized
        ElseIf New Rectangle(Width - 22, 5, 15, 15).Contains(New Point(X, Y)) Then
            If FindForm.ToString.Contains("Form1") Then
                If Form1.connected_yes = 0 Then
                    FindForm.Close()
                Else
                    Try
                        If MsgBox("Voulez-vous vous déconnecter de " + Form1.AppName, MsgBoxStyle.YesNo, Form1.AppName) = vbYes Then
                            Dim g As String
                            ...
                            Form1.connected_yes = 0
                            Form1.InfoBar.ForeColor = Color.FromArgb(253, 106, 72)
                            Form1.InfoBar.Text = "Déconnecté"
                            Form1.InfoBar.Update()
                            Form1.InfoBar.Refresh()
                            Form1.ConnectButton.Text = "Connecter"
                            Form1.ConnectButton.Refresh()
                            Form1.NotifyIcon1.ShowBalloonTip(3000, Form1.AppName, "Status: Déconnecté", ToolTipIcon.Info)
                            End
                        End If
                    Catch ex As Exception
    ...
    Protected Overrides Sub PaintHook()
        ...
        If New Rectangle(Width - 22, 5, 15, 15).Contains(New Point(X, Y)) Then
            G.FillEllipse(New SolidBrush(Color.FromArgb(216, 150, 42)), New Rectangle(Width - 24, 6, 16, 16))
        ...
        If New Rectangle(Width - 44, 5, 15, 15).Contains(New Point(X, Y)) Then
            G.FillEllipse(New SolidBrush(Color.FromArgb(216, 150, 42)), New Rectangle(Width - 44, 6, 16, 16))
            G.DrawString("", New Font("Webdings", 9), New SolidBrush(BackColor), New Point(Width - 43, 5))
        Else
            G.DrawString("", New Font("Webdings", 9), New SolidBrush(Xcolor), New Point(Width - 43, 5))
        End If

        DrawText(New SolidBrush(TitleColor), New Point(10, 9))

        G.DrawLine(New Pen(Color.FromArgb(216, 150, 42), 3), New Point(0, 0), New Point(Width, 0))

        G.DrawLine(New Pen(Color.FromArgb(216, 150, 42), 3), New Point(0, Height), New Point(Width, Height))
    End Sub
End Class
